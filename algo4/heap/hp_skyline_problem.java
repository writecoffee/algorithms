 ...
 *
 */
public class hp_skyline_problem
{
    public class Edge
    {
        public final int     x, height;
        public final boolean isLeftEdge;

        Edge(int x, int height, boolean isLeftEdge)
        {
            this.x = x;
            this.height = height;
    ...
    public List<int[]> getSkyline(int[][] buildings)
    {
        ...
        Queue<Integer> keyPoints = new PriorityQueue<Integer>(new Comparator<Integer>() {
            @Override
        ...
        Queue<Edge> sweep = new PriorityQueue<Edge>(new Comparator<Edge>() {
            @Override
        ...
        for (int[] building : buildings) {
            sweep.add(new Edge(building[0], -building[2], true));
            sweep.add(new Edge(building[1], building[2], false));
        }
        ...
        while (!sweep.isEmpty()) {
            ...
            if (kp.isLeftEdge) {
                keyPoints.add(-kp.height);
            } else {
                keyPoints.remove(kp.height);
            }

            curHighest = keyPoints.peek();
            if (curHighest != preHightest) {
                result.add(new int[] { kp.x, curHighest });
                preHightest = curHighest;
            }
        }
